...
`wire_relaterat.py` + `wire_lang.py` + `wire_identity.py` + `wire_sidfot.py` +
`wire_dates.py` + `generate_llms.py`) och jämför fil för fil mot arbetskopian.
Bara en körning av hela kedjan duger:
**wire-stegen kan ingen enskild generator kontrollera** — tooltips, referenser,
syskonlänkar, språkmärkning, identitet, sidfot och datum läggs på *efter* sidgenereringen, så
