...
- **Norrtou Creations namngivet och länkat på startsidan** (norrtou.se) med korta beskrivningar av och länkar till Aktivitetsdagboken och HEC. Externa länkar följer info.html:s mönster (`target="_blank" rel="noopener noreferrer"`). JSON-LD:s `author.url` pekar nu på norrtou.se med GitHub som `sameAs`, i stället för GitHub som primär url.
