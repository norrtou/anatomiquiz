#!/usr/bin/env python3
"""Generera kärltabeller (artärer/vener) ur data/karl/*.json.

Bygger:
  - kunskapsbank/karl.html            (under-pillar: regionkort)
  - kunskapsbank/karl-<slug>.html     (en regionsida per datafil)

Speglar generate_skelett.py (full <head>, JSON-LD Article/CollectionPage +
BreadcrumbList, APA-referenser, inbyggd tooltip-wiring + körbar SEO §12-grind).
Binomial-korrekt: de fulla latinnamnen (arteria/vena …) ska finnas som
ordlisteposter + i kb_glossary_terms så hela frasen wiras som EN span.
"""
...
<head>
  ...
  <meta property="og:image:alt" content="Anatomiquiz — kroppens kärl (artärer och vener)">
  ...
  <meta name="twitter:image:alt" content="Anatomiquiz — kroppens kärl (artärer och vener)">
...
COLS = [("Kärl (latin)", "c-kla", "latin"), ("Svenska", "c-ksv", "svenska"),
        ("Typ", "c-kty", "typ"), ("Avgår från / mynnar i", "c-kfr", "fran"),
        ("Försörjer / dränerar", "c-kom", "omrade")]

def region_page(d):
    region = d["region"]; slug = d["slug"]
    canon = f"{SITE}/kunskapsbank/karl-{slug}.html"
    title = d.get("titel") or f"{region}s kärl – artärer och vener | Anatomiquiz"
    desc = d.get("desc") or (f"Tabell över {region.lower()}s kärl: artärer och vener med avgång, "
            f"förgreningar och försörjningsområden – för anatomiplugg.")
    crumbs = [("← Anatomiquiz", "/"), ("Kunskapsbank", "/kunskapsbank/"),
              ("Listor & tabeller", "/kunskapsbank/listor-tabeller.html"),
              ("Kärl", "/kunskapsbank/karl.html"),
              (f"{region}s kärl", None)]
    jsonld = {"@context": "https://schema.org", "@type": ["Article", "LearningResource"],
              "headline": f"{region}s kärl – artärer och vener",
    ...
    out.append(f"""    <header class="header">
      <div class="header-title">
        <h1>{esc(d.get("rubrik", region + "s kärl"))}</h1>
      </div>
      <p class="tagline">{esc(d.get("underrubrik", ""))}</p>
    </header>

    <section class="card" aria-labelledby="ktabHeading">
""")
    out.append(crumb(crumbs))
    out.append(f'\n      <h2 id="ktabHeading" class="sr-only">{esc(region)}s kärl</h2>\n')
    ...
    for g in order:
        render_table(g, [v for v in d["karl"] if v.get("grupp") == g])
...
""")
    ...
    out.append(f"""
      <div class="actions">
        <a href="/kunskapsbank/karl.html" class="btn">← Kärl</a>
...
""")
    out.append(FOOT)
    (KB / f"karl-{slug}.html").write_text("".join(out), encoding="utf-8")
    return {
        "slug": slug, "region": region,
        "beskrivning": d.get("kort_beskrivning") or
            f"Artärer och vener i {region.lower()} med avgång och försörjningsområden.",
        "cta": d.get("kort_cta") or f"Till {region.lower()}s kärl →",
    }

def pillar_page(regions):
    canon = f"{SITE}/kunskapsbank/karl.html"
    title = "Kärl – artärer och vener per region | Anatomiquiz"
    desc = ("Kroppens kärl per region: artärer och vener med avgång, förgreningar och "
            "försörjningsområden – för tentaplugg i anatomi. Välj region nedan.")
    crumbs = [("← Anatomiquiz", "/"), ("Kunskapsbank", "/kunskapsbank/"),
              ("Listor & tabeller", "/kunskapsbank/listor-tabeller.html"),
              ("Kärl", None)]
    jsonld = {"@context": "https://schema.org", "@type": ["CollectionPage", "LearningResource"],
              "name": "Kärl – artärer och vener per region", "description": desc,
              "inLanguage": "sv-SE", "isAccessibleForFree": True, "learningResourceType": "reference",
              "isPartOf": {"@type": "WebSite", "name": "Anatomiquiz", "url": f"{SITE}/"},
              "hasPart": [{"@type": "LearningResource", "name": f"{r['region']}s kärl",
                           "url": f"{SITE}/kunskapsbank/karl-{r['slug']}.html"} for r in regions],
              "breadcrumb": bc_jsonld([(n if n != "← Anatomiquiz" else "Anatomiquiz", u or canon) for n, u in crumbs])}
    ...
    out.append("""    <header class="header">
      ...
      <p class="tagline">Artärer och vener – avgång, förgreningar och försörjningsområden, per region</p>
    </header>

    <section class="card" aria-labelledby="kpHeading">
""")
    out.append(crumb(crumbs))
    out.append('\n      <h2 id="kpHeading" class="sr-only">Kärl per region</h2>\n')
    out.append('      <div class="info-about">\n        <p>Blodkärlen presenteras region för region: artärerna med varifrån de avgår och vad de försörjer, venerna med vad de dränerar och var de mynnar. Välj en region för att se hela tabellen.</p>\n      </div>\n')
    out.append('      <div class="kb-grid">\n')
    for r in regions:
        out.append(f"""        <a class="kb-card" href="/kunskapsbank/karl-{r['slug']}.html">
          <h3 class="kb-card-title">{esc(r['region'])}s kärl</h3>
...
""")
    out.append(FOOT)
    (KB / "karl.html").write_text("".join(out), encoding="utf-8")

def main():
    ORDER = ["huvud-hals", "armen", "brostkorgen", "buken", "backenet", "benet"]
    ...
    for f in ordered:
        ...
        r = region_page(d)
        regions.append(r)
        generated.append(KB / f"karl-{r['slug']}.html")
        print(f"  karl-{r['slug']}.html ({r['region']}, {len(d['karl'])} kärl)")
    pillar_page(regions)
    generated.append(KB / "karl.html")
    print(f"  karl.html (under-pillar, {len(regions)} region(er))")
